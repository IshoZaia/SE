...
const app = express();
app.use(cors({
    origin: 'http://localhost:5001', // Allow requests from frontend
...
async function processCourses(frequency) {
  try {
    ...
    // Fetch courses with the specified frequency
    const courses = await Course.find({ emailFrequency: frequency })
      .populate('file')
      .populate('creator', 'username email _id') // Populate creator details
      .populate('members.userId', 'username email _id'); // Populate members' details
